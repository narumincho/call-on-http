...
export const middleware = (request: a.Request, response: a.Response): void => {
  ...
  const f = (l: number, m: Uint8Array): { result: number, nextIndex: number }=>{
    let n: number = 0;
for (let o = 0; o < 5;o+= 1){
  ...
  const i = (l: number, m: Uint8Array): { result: string, nextIndex: number }=>{
    const n: { result: number, nextIndex: number } = f(l, m);
    return { result: new b.TextDecoder().decode(m.slice(l + n.nextIndex, l + n.nextIndex + n.result)), nextIndex: l + n.nextIndex + n.result };
  };
  const j = (l: number, m: Uint8Array): { result: createUserRequest, nextIndex: number }=>{
    const n: { result: number, nextIndex: number } = f(l, m);
    ...
    if (o === 0) {
      ...
      return { result: q.nextIndex, nextIndex: { name: p.result, age: q.result } };
    }
    throw new Error("サポートされていなパターンです");
  };
  const k = (l: number, m: Uint8Array): { result: getUserRequestObject, nextIndex: number }=>{
    const n: { result: number, nextIndex: number } = f(l, m);
    const o: number = n.result;
    if (o === 0) {
      const p: { result: number, nextIndex: number } = "IDはまだサポートしていない"(n.nextIndex, m);
 ...
 * @param request 
 */
export const createUser = (request: createUserRequest): string => "createUser@1"
